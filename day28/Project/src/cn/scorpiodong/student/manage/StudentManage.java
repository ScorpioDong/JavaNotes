...
public class StudentManage {
     ...
     * 学生类数组，存储Student类对象
     */
    private ArrayList<Student> allStus;

    /**
     * 使用默认容量初始化学生数组
     */
    public StudentManage() {
        allStus = new ArrayList<>();
    }
    ...
    public StudentManage(int initCapacity) {
        allStus = new ArrayList<>(initCapacity);
    }
     ...
     * 显示学生数组所有学生的信息
     */
    public void show(int begin, int length) {
        if (begin < 0 || begin > allStus.size() - 1) {
            throw new IndexOutOfBoundsException(String.valueOf(begin));
        }

        int endIndex = begin + length;
        if (allStus.size() - begin - 1 < length) {
            endIndex = allStus.size();
        }
    ...
    public void filter(Predicate<Student> filter) {
        // 符合条件的学生数组
    ...
    public void setAllStus(ArrayList<Student> allStus) {
