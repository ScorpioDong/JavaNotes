...
public class StudentManage {
    ...
    public boolean save() {
        ...
        try {
            ...
            String counter = String.format("%-10d", Student.getCount());

            writer.write(counter);

            for (int i = 0; i < allStus.size(); i++) {
                String dataFormat = String.format("|%-10d|%-10s|%-10d|%-10c|%-10d|%-10d|%-10d|%-10d|%-10d|",
                        allStus.get(i).getId(),
                        allStus.get(i).getName(),
                        allStus.get(i).getAge(),
                        allStus.get(i).getGender(),
                        allStus.get(i).getMathScore(),
                        allStus.get(i).getChnScore(),
                        allStus.get(i).getEngScore(),
                        allStus.get(i).getTotalScore(),
                        allStus.get(i).getRank());
    ...
    public boolean load() {
        ...
        try {
            ...
            reader.read(arr, 0, 10);

            int length = -1;

            char[] data = new char[100];

            while ((length = reader.read(data, 0, 100)) != -1) {
                String[] dataFormat = new String(data).split("\\|");

                Student student = new Student();

                student.setId(Integer.parseInt(dataFormat[1].replace('|',' ').trim()));
                student.setName(dataFormat[2].replace('|',' ').trim());
                student.setAge(Integer.parseInt(dataFormat[3].replace('|',' ').trim()));
                student.setGender(dataFormat[4].replace('|',' ').trim().charAt(0));
                student.setMathScore(Integer.parseInt(dataFormat[5].replace('|',' ').trim()));
                student.setChnScore(Integer.parseInt(dataFormat[6].replace('|',' ').trim()));
                student.setEngScore(Integer.parseInt(dataFormat[7].replace('|',' ').trim()));
                student.setTotalScore(Integer.parseInt(dataFormat[8].replace('|',' ').trim()));
                student.setRank(Integer.parseInt(dataFormat[9].replace('|',' ').trim()));

                add(student);
            }

            Student.setCount(Integer.parseInt(new String(arr).trim()));
